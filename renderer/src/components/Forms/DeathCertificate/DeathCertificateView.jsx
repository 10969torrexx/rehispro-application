...
export default function DeathCertificateView({ row }) {
    ...
    const handleDownload = async (id) => {
        try {
            ...
            await DeathCertServices.download(id);
        ...
        <>
            {
                loading ? (
                    <LoadingScreen 
                        title={isDownloading ? 'Extracting data to PDF' : ''} 
                        message={isDownloading ? "This might take sometime." : ''} 
                    />
                ) : (
                    <>
                        <div className='w-full flex justify-end'>
                            <button 
                                className='m-x-[200px] btn-success shadow-lg text-white px-3 py-1 rounded-full'
                                onClick={() => handleDownload(deceased.id)}    
                            >
                                <i className="fa-solid fa-download mr-2"></i> Extract to PDF 
                            </button>
                        </div>
